...
A_P_theory = p.readout.DAStdpRate * p.readout.Mu * p.input.rewPulseScale / (p.input.rewTau * exp(1) * ep.DTsim * lambda_theory)

print "A_P_theory = ", A_P_theory
...
for exp_num in range(len(filenames)):
    ...
    startRewBin = pars[exp_num].input.initT # + pars[exp_num].input.rewardT
...
totalNumTrials = ep.nTrainEpochs + 2*ep.nTestEpochs - 2*ep.numTrialsWithoutThreshold
...
for tmpl_i in range(p.input.nTemplates):        
    ...
    print " value is ", 2*int(p.input.targetTemplate == tmpl_i)
...
savefig("./pattern_weight_depend.eps")

...
def display_spike_trains(f, idx, XBeforeMin, XBeforeMax, ax):
    ...
    xlim(0, XBeforeMax - XBeforeMin + 0.004)
    xticks( arange(0,XBeforeMax - XBeforeMin+0.004,0.4), [ "%.1f" % i for i in arange(0,XBeforeMax - XBeforeMin+0.04,0.4) ] ) 
    xlabel("time [sec]")    
    ylim(0,8)
    yticks([])
...
def plot_spike_corr(corr, p):
    ...
    xticks( arange(0,len(corr)) , [ "%d" % i for i in arange(0, int(ep.Tsim/60)+0.002, int(ep.Tsim/60/6) ) ] )
    xlim(0,6.1)
    xlabel('time [min]' ) 
    ylabel('spike correlation')
    yticks( arange(0.50,0.95,0.1), [ '%.1f' % (x,) for x in arange(0.5,0.95,0.1) ] )
    ylim(0.50,0.851)
...
def plot_weight_change_fig(r, p):    
    ...
    print " Number of failed strong = ", failed_strong, " Number of failed weak = ", failed_weak
...
if __name__ == "__main__":
    ...
    if mode == 'complete':
        if len(sys.argv) > 1:
            ...
            sim_dir = last_created_dir('wstar.*')
        sim_file = os.path.join(sim_dir, last_file('biofeed_final_2.*', sim_dir))    
